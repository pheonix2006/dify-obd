...
class DualWorkflowComparator:
    ...
    async def compare_answers(
        ...
        try:
            ...
            if self.recorder and question_index is not None:
                logger.info(
                    f"[评测记录] 开始保存双工作流对比评测记录，问题索引: {question_index}"
                )
                parsed_result = {
                ...
                if record_path:
                    ...
                    logger.warning(
                        f"[评测记录] 评测记录保存失败，recorder.enabled={self.recorder.enabled}"
                    )
            elif self.recorder is None:
                logger.debug("[评测记录] recorder 为 None，未保存评测记录")
            elif question_index is None:
                logger.debug("[评测记录] question_index 为 None，未保存评测记录")
    ...
    async def _call_llm_api(self, prompt: str) -> str:
        ...
        try:
            ...
            if self.config.api_type == "custom_azure":
                ...
                if "output" in result and len(result["output"]) > 0:
                    ...
                    if output_content and isinstance(output_content, list):
                        text_value = output_content[0].get("text", "")
            ...
            if not content:
                logger.error(
                    f"API 返回格式异常: {json.dumps(result, ensure_ascii=False)}"
                )
                return ""
    ...
    def _parse_comparison_response(
        ...
        logger.debug(
            f"解析标签: label_1={label_1}, label_2={label_2}, label_history={label_history}"
        )
        ...
        confidence_match = re.search(
            r"\*{0,2}置信度\*{0,2}[：:]\s*(high|medium|low)", content, re.IGNORECASE
        )
        if confidence_match:
        ...
        # 日志记录解析结果
        logger.debug(
            f"解析结果: winner={winner}, confidence={confidence}, "
