...
class WorkflowBatchProcessor:
    ...
    # 类属性类型注解
    semantic_judge: Optional["SemanticJudge"]
    dual_workflow_comparator: Optional[DualWorkflowComparator]
    ...
    def __init__(
        ...
        dual_workflow_schema_config: Optional[
            "DualWorkflowSchemaConfig"
        ] = None,  # 新增
    ):
        self.config = config
        ...
        if self.llm_eval_config.eval_record_enabled:
            self.eval_recorder = EvalRecorder(
                base_dir=self.llm_eval_config.eval_record_path, enabled=True
            )
            logger.info(
                f"评测记录器已启用，记录目录: {self.eval_recorder.get_session_dir()}"
            )
        ...
        if llm_eval_config:
            ...
            self.semantic_judge = SemanticJudge(
                llm_eval_config, recorder=self.eval_recorder  # 传递记录器
            )
        else:
        ...
        if dual_workflow_config and llm_eval_config:
            self.dual_workflow_comparator = DualWorkflowComparator(
                llm_eval_config, recorder=self.eval_recorder
            )
        else:
    ...
    def _load_rag_results_from_df(self, df: "pd.DataFrame") -> List[QuestionAnswer]:
        ...
        for _, row in df.iterrows():
            ...
            qa = QuestionAnswer(
                ...
                original_index=(
                    int(row.get("序号", 0)) - 1 if pd.notna(row.get("序号")) else None
                ),
                ...
                error=(
                    row.get("错误信息")
                    if pd.notna(row.get("错误信息")) and row.get("错误信息") != ""
                    else None
                ),
            )
            ...
            qa.improvement_analysis = (
                improvement_val if pd.notna(improvement_val) else None
            )
    ...
    def _load_standard_results_from_df(
    ...
    def _load_dual_workflow_results_from_df(
        self, df: "pd.DataFrame"
    ) -> List[QuestionAnswer]:
        ...
        # 优先使用配置中的标签，否则从列名推断
        label_1 = (
            self.dual_workflow_config.label_1
            if self.dual_workflow_config
            else "Workflow_A"
        )
        label_2 = (
            self.dual_workflow_config.label_2
            if self.dual_workflow_config
            else "Workflow_B"
        )
        ...
        if not col_workflow_1 or not col_workflow_2:
            answer_cols = [
                col for col in df.columns if "回答" in col or "Answer" in col
            ]
            # 排序后取前两个（通常工作流1在前）
        ...
        logger.debug(
            f"推断的工作流列: workflow_1='{col_workflow_1}', workflow_2='{col_workflow_2}'"
        )
        ...
        for _, row in df.iterrows():
            ...
            # 提取召回片段（rerank_sources）
            rerank_sources = row.get("召回片段", "")
            if pd.isna(rerank_sources):
                rerank_sources = None

            # 提取历史回答（history_answer）
            history_answer = row.get("历史回答", "")
            if pd.isna(history_answer) or history_answer == "":
                history_answer = None
    ...
    async def _save_incremental_all(
        self, results: List[QuestionAnswer], output_path: str
    ):
        """增量保存所有当前结果"""
        async with self._file_lock:
            ...
            sorted_results = sorted(
                results,
                key=lambda x: x.original_index if x.original_index is not None else 0,
            )
    ...
    async def _process_row_logic(
        self, row: pd.Series, idx: int, total_rows: int
    ) -> Optional[QuestionAnswer]:
        """单行处理的具体逻辑"""
        # 提取列值
        problem_val = row.get(self.routing_config.problem_value_column, "")
        if pd.isna(problem_val) or (
            isinstance(problem_val, str) and not problem_val.strip()
        ):
            return None
        ...
        if not api_key:
            ...
            qa.error = f"未找到知识库 '{knowledge_base}' 对应的 API Key"
            return qa

        # 创建QuestionAnswer对象
        qa = QuestionAnswer(question=question, expected_answer=str(expected_answer))
        qa.knowledge_base = knowledge_base
        qa.used_api_key = api_key[-4:]
        qa.original_index = idx
        ...
        try:
            ...
            result = await temp_client.execute_workflow(inputs, self.config.user, None)

            qa.workflow_run_id = result.get("task_id")
            actual_answer = result.get(
                self.config.output_variable_name, json.dumps(result, ensure_ascii=False)
            )
        ...
        if actual_answer and not qa.error:
            ...
            if answer_state is not None:
                if isinstance(answer_state, str):
                    is_eval_mode = answer_state.strip().lower() not in (
                        "false",
                        "0",
                        "",
                    )
    ...
    async def _process_excel_new_mode(
        ...
        if output_path and os.path.exists(output_path):
            ...
            if existing_results:
                ...
                print(
                    f"检测到断点：已从现有结果中加载了 {len(existing_results)} 条已处理记录。"
                )

        # 确定真正需要处理的行
        remaining_indices = [
            i for i in range(start_row, end_row) if i not in processed_indices
        ]
        ...
        if output_path:
            saver_task = asyncio.create_task(
                self._results_saver(
                    result_queue, results, output_path, stop_saver_event
                )
            )
        ...
        async def process_task(idx):
            ...
            if qa.workflow_result and not qa.error:
                is_match, match_type = self.comparator.compare(
                    expected_answer, qa.workflow_result, method=comparison_method
                )
    ...
    def calculate_statistics(self, results: List[QuestionAnswer]) -> Dict[str, Any]:
        ...
        # 异常模式数量
        feedback_mode_count = sum(
            1 for qa in results if not qa.is_evaluated and qa.error is None
        )
        ...
        for qa in evaluated_results:
            if qa.llm_category:
                category_stats[qa.llm_category] = (
                    category_stats.get(qa.llm_category, 0) + 1
                )
        ...
        if evaluated_count > 0:
            for category, count in category_stats.items():
                category_percentages[str(category)] = count / evaluated_count

        # 4级分类详情
        category_details: Dict[str, Dict[str, Any]] = {}
        for cat_enum in AnswerCategory:
            cat_val: str = cat_enum.value
            category_details[cat_val] = {
                "count": category_stats.get(cat_val, 0),
                "percentage": category_percentages.get(cat_val, 0.0),
                "label": cat_enum.label,
            }
        ...
        for qa in evaluated_results:
            if qa.match_type:
                match_type_stats[qa.match_type] = (
                    match_type_stats.get(qa.match_type, 0) + 1
                )
    ...
    def _calculate_dual_workflow_stats(
        self, results: List[QuestionAnswer]
    ) -> Dict[str, Any]:
        """
    ...
    def _save_results_standard(
        ...
        for qa in results:
            ...
            display_idx = (
                (qa.original_index + 1)
                if qa.original_index is not None
                else len(data) + 1
            )
            ...
            data.append(
                {
                    ...
                    "LLM 评测提示词": (
                        qa.llm_eval.prompt
                        if (qa.llm_eval and qa.llm_eval.prompt)
                        else ""
                    ),
                }
            )
    ...
    def _save_results_dual_workflow(
        ...
        label_1 = (
            self.dual_workflow_config.label_1 if self.dual_workflow_config else "LLM1"
        )
        label_2 = (
            self.dual_workflow_config.label_2 if self.dual_workflow_config else "LLM2"
        )
        label_history = (
            self.dual_workflow_config.label_history
            if self.dual_workflow_config
            else "历史回答"
        )
        ...
        with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
            ...
            stats_data.append(
                [f"{label_history}获胜次数", statistics.get("history_wins", 0)]
            )
            stats_data.append(["平局次数", statistics.get("ties", 0)])
            stats_data.append(["", ""])  # 分隔行
            stats_data.append(["--- 获胜率 ---", ""])
            stats_data.append(
                [f"{label_1}获胜率", f"{statistics.get('llm1_win_rate', 0):.2%}"]
            )
            stats_data.append(
                [f"{label_2}获胜率", f"{statistics.get('llm2_win_rate', 0):.2%}"]
            )
            stats_data.append(
                [
                    f"{label_history}获胜率",
                    f"{statistics.get('history_win_rate', 0):.2%}",
                ]
            )
    ...
    async def _process_excel_standard_mode(
        ...
        if is_new_mode:
            print("检测到路由配置列，使用路由分发 + 评测分支模式")
            return await self._process_excel_new_mode(
                df, start_row, end_row, delay, output_path
            )
        else:
            print("使用传统模式：单API Key + 全量评测")
    ...
    async def _process_excel_rag_eval_mode(
        ...
        if schema is None:
            raise ValueError(
                "rag_eval_schema_config is None, cannot process in rag_eval mode"
            )
                ...
                try:
                    ...
                    async with semaphore:
                        ...
                        if workflow_result:
                            ...
                            extracted_question = (
                                parsed.question if parsed.question else None
                            )
                            rerank_sources = (
                                parsed.rerank_sources if parsed.rerank_sources else None
                            )
                            llm_answer = (
                                parsed.llm_answer if parsed.llm_answer else None
                            )
    ...
    async def _call_dify_api_for_rag(self, question: str) -> str:
        ...
        try:
            ...
            if isinstance(result, dict):
                output = result.get(self.config.output_variable_name, "")
    ...
    async def _call_dify_api_dual(self, question: str):
        """
        ...
        # 验证配置
        if self.dual_workflow_config is None:
            raise ValueError("dual_workflow_config is None")
        ...
        try:
            ...
            if isinstance(result, dict):
                ...
                if "data" in result and isinstance(result["data"], list):
                    logger.debug(
                        f"[双工作流] 检测到流式响应，data 条目数: {len(result['data'])}"
                    )
            ...
            # 调试日志：记录解析结果状态
            logger.debug(
                f"[双工作流] 解析状态: is_valid_format={parsed.is_valid_format}"
            )

            # 召回片段提取状态
            if parsed.rerank_sources:
                logger.info(
                    f"[双工作流] 已提取召回片段（长度: {len(parsed.rerank_sources)} 字符）"
                )
            else:
            ...
            async def run_task(idx):
                ...
                try:
                    ...
                    async with semaphore:
                        ...
                        # 三方对比评测
                        eval_result = None
                        if self.dual_workflow_comparator and self.dual_workflow_config:
                    ...
                    qa = QuestionAnswer(
                        ...
                        rerank_sources=(
                            parts.rerank_sources if parts.is_valid_format else None
                        ),
                        history_answer=history_answer,
                        winner=eval_result.winner if eval_result else None,
                        comparison_analysis=(
                            eval_result.overall_analysis if eval_result else None
                        ),
