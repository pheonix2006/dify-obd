...
class TestWorkflowBatchProcessor:
    ...
    @pytest.mark.asyncio
    async def test_load_excel_invalid_column(self, processor, sample_excel_file):
        """测试Excel文件中缺少必需列"""
        # 重命名question列
        df = processor.load_excel(sample_excel_file)
        ...
        # 列检查是在 process_excel -> _process_excel_legacy_mode 中进行的
        with pytest.raises(ValueError, match="Excel文件中不存在列: question"):
            await processor.process_excel(sample_excel_file)

    @pytest.mark.asyncio
    async def test_process_question_success(self, processor):
        """测试成功处理单个问题"""
        # 设置mock
        ...
        # 处理问题
        question = "测试问题"
        result = await processor_with_mock.process_question(question, input_variable_name="query", output_variable_name="answer")
    ...
    @pytest.mark.asyncio
    async def test_process_question_without_output_var(self, processor):
        """测试工作流输出没有指定变量名的情况"""
        # 设置mock - 输出中没有指定的变量名
        from unittest.mock import AsyncMock
        mock_client = AsyncMock()
        ...
        # 处理问题
        question = "测试问题"
        result = await processor_with_mock.process_question(question)
    ...
    @pytest.mark.asyncio
    async def test_process_question_api_error(self, processor):
        """测试API调用错误"""
        # 设置mock - API调用失败
        from unittest.mock import AsyncMock
        mock_client = AsyncMock()
        mock_client.execute_workflow = AsyncMock(side_effect=Exception("API调用失败"))
    ...
    @pytest.mark.asyncio
    async def test_process_excel_basic(self, sample_config):
        """测试基本Excel处理"""
        ...
        with patch('obd.processor.batch_processor.WorkflowBatchProcessor.load_excel', return_value=mock_df):
            with patch('obd.processor.batch_processor.WorkflowBatchProcessor.process_question', side_effect=mock_process_question):
                # 处理Excel
    ...
    @pytest.mark.asyncio
    async def test_process_excel_with_range(self, processor):
        """测试处理指定范围的Excel"""
        from unittest.mock import patch

        # 设置mock DataFrame - 5行数据
        mock_df = pd.DataFrame({
            'question': [f'问题{i+1}' for i in range(5)],
            'answer': [f'答案{i+1}' for i in range(5)]
        })
        ...
        with patch('obd.processor.batch_processor.WorkflowBatchProcessor.load_excel', return_value=mock_df):
            ...
            async def mock_process_question(*args, **kwargs):
                call_count[0] += 1
                qa = QuestionAnswer(
                    question=f"问题{call_count[0]}",
                    expected_answer=f"答案{call_count[0]}",
                    workflow_result=f"结果{call_count[0]}",
                    is_correct=True
                )
                return qa
    ...
    @pytest.mark.asyncio
    async def test_process_excel_all_columns_not_found(self, processor):
        """测试Excel中缺少必需列"""
        from unittest.mock import patch
        ...
        with patch('obd.processor.batch_processor.WorkflowBatchProcessor.load_excel', return_value=mock_df):
            # 应该抛出ValueError
            with pytest.raises(ValueError, match="Excel文件中不存在列: question"):
                await processor.process_excel("dummy_path.xlsx")
